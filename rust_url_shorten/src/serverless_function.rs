...
use std::hash::Hasher;
use crate::value_parser::{Value, serialize_value, deserialize_value};
...
pub fn serverless(req: HttpRequest) -> HttpResponse {
        ...
        match kv::get(&key) {
            Ok(val) => {
                ...
                if value.password.is_none() {
                    ...
                    false
                } else if password.is_some() && password.as_ref().unwrap() == value.password.as_ref().unwrap() {
                    // Passwords match
                    true
                } else {
    ...
    if key_available(&shortened) || authenticate(&shortened, &old_password) {
        ...
        match kv::put(&shortened, val, None) {
            Err(e) => {
                HttpResponse::from(format!("{}", e)).set_status(StatusCode::BAD_REQUEST)
            }
